...
class ProductGallery extends HTMLElement {
    constructor() {
        ...
        this.thumbBtns = [];
        this.lightboxThumbBtns = [];
        this.focusableEls = [];
    }
    connectedCallback() {
        const sourceImgs = Array.from(this.querySelectorAll(':scope > img'));
    ...
    buildDOM() {
        ...
        if (multi) {
            this.thumbBtns = this.images.map((img, i) => this.makeThumbBtn(img, i, this.images.length));
        ...
        if (multi) {
            this.lightboxThumbBtns = this.images.map((img, i) => this.makeThumbBtn(img, i, this.images.length));
    ...
    bindEvents() {
        ...
        this.mq.addEventListener('change', () => { if (!this.mq.matches && this.lightboxOpen)
            this.closeLightbox(); }, listenerOpts);
    }
    step(dir) {
    ...
    queryFocusable() {
        return Array.from(this.lightboxEl.querySelectorAll(ProductGallery.FOCUSABLE_SELECTOR)).filter((el) => !el.closest('[hidden]') && el.offsetParent !== null);
    }
    onKeydown(e) {
        if (!this.lightboxOpen)
            return;
    ...
    trapFocus(e) {
        if (e.key !== 'Tab' || !this.focusableEls.length)
            return;
...
ProductGallery.FOCUSABLE_SELECTOR = 'a[href], button:not([disabled]), input:not([disabled]), ' +
